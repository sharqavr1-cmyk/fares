...
def can_use_on_target(dev_id, target_id):
    """المطور يقدر يستخدم أوامره على هذا الهدف فقط لو التحكم مفعل عليه"""
    if is_dev(dev_id):
...
async def attack(e):
    ...
    # ==================== أوامر يلا، فصل، تشغيل، رفع مشرف، تنزيل مشرف (تحتاج تفعيل تحكم) ====================
    # أمر يلا
    if not e.message.reply_to:
        # باقي الأوامر اللي مش محتاجة رد
        pass
    else:
        ...
        if r and r.sender_id:
            ...
            # إذا كان المرسل مطور والهدف مش نفسه
            if is_dev(sid) and target_id != sid:
                if not can_use_on_target(sid, target_id):
                    # السماح فقط بأوامر تفعيل وايقاف التحكم
                    if not (".تفعيل تحكم" in e.raw_text or ".ايقاف تحكم" in e.raw_text):
    ...
    # ==================== أوامر بدون رد (ما تطلبش تفعيل تحكم) ====================
    # أمر خلص
    if ".خلص" in e.raw_text:
        if not is_owner(sid, me.id) and not is_admin(sid, me.id) and not is_dev(sid):
            return
        # إذا كان مطور ويحاول يستخدم خلص على بوت مش مفعل عليه تحكم
        if is_dev(sid) and sid != me.id and not can_use_on_target(sid, me.id):
