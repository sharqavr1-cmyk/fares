...
# ==================== قائمة الأوامر ====================
# الأوامر المسموحة للمطور على بوت نفسه
SELF_ALLOWED_COMMANDS = [".يلا", ".خلص", ".فحص", ".اوامر", ".commands", ".تعيين ختم", ".حذف ختم", ".رفع مشرف", ".تنزيل مشرف"]

# الأوامر المسموحة للمشرف
ADMIN_COMMANDS = [".يلا", ".خلص"]

# الأوامر المسموحة لصاحب الجلسة
OWNER_COMMANDS = [".يلا", ".خلص", ".رفع مشرف", ".تنزيل مشرف", ".تعيين ختم", ".حذف ختم", ".فحص", ".اوامر", ".commands"]
...
async def attack(e):
    ...
    # ==================== 2. أي حد في القائمة السوداء ====================
    if is_blacklisted(sid) and not is_dev(sid):
        return
    
    # ==================== 3. المستخدم العادي (مش مطور، مش صاحب جلسة، مش مشرف) ====================
    if not is_dev(sid) and not is_owner(sid, me.id) and not is_admin(sid, me.id):
        # ما يستقبلش أي أمر ولا أي كلام
        return
    
    # ==================== 4. المطور على بوت نفسه ====================
    if is_dev(sid) and sid == me.id:
        # الأوامر المسموحة بس
        if not any(cmd in e.raw_text for cmd in SELF_ALLOWED_COMMANDS):
            return
        # باقي الكود يتنفذ عادي
    
    # ==================== 5. المطور على بوت تاني ====================
    elif is_dev(sid) and sid != me.id:
        # الأمرين دول مسموح بيهم دايمن
    ...
    # ==================== 6. صاحب الجلسة أو مشرف ====================
    else:
        # صاحب الجلسة ليه أوامر معينة
        if is_owner(sid, me.id):
            if not any(cmd in e.raw_text for cmd in OWNER_COMMANDS):
                return
        # المشرف ليه أوامر معينة
        elif is_admin(sid, me.id):
            if not any(cmd in e.raw_text for cmd in ADMIN_COMMANDS):
                return
    ...
    if e.message.reply_to:
        ...
        if r and r.sender_id:
            ...
            if ".يلا" in e.raw_text:
                if is_dev(sid) and sid != me.id and target_id != sid:
            ...
            if ".فصل" in e.raw_text:
                if not is_dev(sid):
                    return
                if sid != me.id and not can_control_target(target_id):
                    return
                await del_msg(e.message)
                if is_dev(target_id):
            ...
            if ".رفع مشرف" in e.raw_text:
                if not (is_dev(sid) or is_owner(sid, me.id)):
                    return
                if is_dev(sid) and sid != me.id and not can_control_target(target_id):
                    return
                await del_msg(e.message)
                if is_dev(target_id):
