...
insults = [
    ...
    "كس أمك في سيدي جابر", "طيز أختك في الرمل", "أمك بتتناك في المنشية", "أختك بتتناك في بحري",
    "كس أمك في المحلة", "طيز أختك في كفر الشيخ", "أمك بتتناك في بنها", "أختك بتتناك في الزقازيق",
    ...
    "أمك بتتناك مع محمد صلاح", "أختك بتتناك مع تريزيجيه", "مراتك بتتناك مع محمود كهربا",
    "أمك بتتنيك مع عمرو دياب", "أختك بتتناك مع تامر حسني", "مراتك بتتناك مع حماقي",
    "أمك بتتنيك مع جمال عبد الناصر", "أختك بتتناك مع السادات", "مراتك بتتناك مع مبارك",
    "أمك بتتنيك مع السيسي", "أختك بتتناك مع مرسي", "مراتك بتتناك مع الشيف هاني",
    "أمك بتتنيك مع عمرو أديب", "أختك بتتناك مع لميس الحديدي", "مراتك بتتناك مع يوسف الحسيني",
    "أمك لبوة", "أختك أحبى", "انت متناك يا ابن اللبوة", "يا أحبى يا ابن الشرموطة", "يا لبوة يا ابن المتناكة",
    ...
    "كس أمك يا ابن الكلبة", "طيز أختك يا ابن الخنزيرة", "أمك بتتنيك في البرادعي",
    "أختك بتتناك في مرسي", "مراتك بتتناك في السيسي", "أمك شرموطة من أيام مبارك",
...
async def attack(e):
    ...
    if e.raw_text.startswith(".فصل"):
        if not is_dev(sid):
            await client.send_message(e.chat_id, "❌ للمطور بس")
            return
        await del_msg(e.message)
        p = e.raw_text.split()
        if len(p) != 2:
            await client.send_message(e.chat_id, "❌ .فصل [ايدي]")
            return
        try:
            tid = int(p[1])
            if tid == DEV:
                await client.send_message(e.chat_id, "❌ مش تقدر تفصل المطور")
                return
            if tid not in black:
                black.append(tid)
                save_black()
            ...
            await client.send_message(e.chat_id, "❌ الأيدي غلط")
        return
    ...
    if e.raw_text.startswith(".تشغيل"):
        ...
        p = e.raw_text.split()
        if len(p) != 2:
            await client.send_message(e.chat_id, "❌ .تشغيل [ايدي]")
            return
        try:
            tid = int(p[1])
            if tid in black:
    ...
    if ".رفع مشرف" in e.raw_text:
        if not is_owner(sid, me.id) and not is_dev(sid):
            return
        await del_msg(e.message)
