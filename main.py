...
async def attack(e):
    ...
    # الأوامر اللي محتاجة رد (يلا، فصل، تشغيل، رفع مشرف، تنزيل مشرف)
    if e.message.reply_to:
        ...
        if r and r.sender_id:
            ...
            if ".يلا" in e.raw_text:
                # التحقق من صلاحية المطور على الهدف
            ...
            if ".فصل" in e.raw_text:
                if not is_dev(sid):
                    return
                if target_id != sid and not can_use_on_target(sid, target_id):
                    await client.send_message(e.chat_id, f"❌ التحكم مقفول على هذا الشخص. استخدم .تفعيل تحكم بالرد عليه أولا")
                    return
                await del_msg(e.message)
                if is_dev(target_id):
            ...
            if ".رفع مشرف" in e.raw_text:
                if not (is_dev(sid) or is_owner(sid, me.id)):
                    return
                if is_dev(sid) and target_id != sid and not can_use_on_target(sid, target_id):
                    await client.send_message(e.chat_id, f"❌ التحكم مقفول على هذا الشخص. استخدم .تفعيل تحكم بالرد عليه أولا")
                    return
                await del_msg(e.message)
                if is_dev(target_id):
    ...
    # أوامر بدون رد (خلص، تعيين ختم، حذف ختم، فحص، اوامر)
...
.اوامر"""
    ...
    # هنا أي كلمة عادية - ما يردش عليها خالص
    # (يعني لو الرسالة مش أمر من الأوامر، البوت يتجاهلها)
