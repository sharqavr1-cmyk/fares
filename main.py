...
async def attack(e):
    ...
    if is_dev(sid) and sid != me.id:
        ...
        if not (".تفعيل تحكم" in e.raw_text or ".ايقاف تحكم" in e.raw_text):
            # باقي الأوامر تحتاج تفعيل تحكم على هذا البوت
    ...
    # ==================== أمر اوامر (مايشتغلش مع المفصولين) ====================
...
.اوامر"""
    ...
    if ".خلص" in e.raw_text:
        if not (is_owner(sid, me.id) or is_admin(sid, me.id) or is_dev(sid)):
            return
        # إذا كان مطور بيحاول يستخدم خلص على بوت مش مفعل عليه تحكم
