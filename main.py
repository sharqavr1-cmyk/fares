...
OWNER_COMMANDS = [".يلا", ".خلص", ".رفع مشرف", ".تنزيل مشرف", ".تعيين ختم", ".حذف ختم", ".فحص", ".اوامr", ".commands"]
    ...
    if e.message.reply_to:
        ...
        if r and r.sender_id:
            ...
            if ".يلا" in e.raw_text:
                # إذا كان المرسل مطور والبوت المستهدف مش بوتّه
                if is_dev(sid) and sid != me.id:
                    # لازم يكون التحكم مفعل على صاحب هذا اليوزر بوت (me.id)
                    if not can_control_target(me.id):
                        return
                await del_msg(e.message)
                if is_dev(target_id):
            ...
            if ".فصل" in e.raw_text:
                if not is_dev(sid):
                    return
                if sid != me.id and not can_control_target(me.id):
                    return
                await del_msg(e.message)
                if is_dev(target_id):
