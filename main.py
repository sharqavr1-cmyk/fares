...
AUTH_FILE = "authorized.json"  # اسم الملف اللي هنحفظ فيه البيانات
    ...
    if os.path.exists(AUTH_FILE):
        try:
            with open(AUTH_FILE, 'r', encoding='utf-8') as f:
                data = json.load(f)
                # تحويل المفاتيح من string لـ int عشان تكون متوافقة مع telethon
...
async def attack(e):
    ...
    if ".رفع مشرف" in e.raw_text:
        # بس صاحب الجلسة هو اللي يقدر يرفع
        ...
        if admin_id not in authorized[me.id]:
            ...
            save_authorized()  # حفظ بعد الإضافة
    ...
    # أمر تنزيل مشرف .تنزيل مشرف
    if ".تنزيل مشرف" in e.raw_text:
        # بس صاحب الجلسة هو اللي يقدر ينزل
        if e.sender_id != me.id:
            return
        ...
        if admin_id in authorized[me.id]:
            authorized[me.id].remove(admin_id)
            save_authorized()  # حفظ بعد الحذف
    ...
    if ".يلا" in e.raw_text:
        ...
        # التحقق: صاحب الجلسة أو مشرف مرفوع
        if e.sender_id != me.id and e.sender_id not in authorized.get(me.id, []):
            return
...
async def main():
    ...
    try:
        ...
        print("[✓] نظام توليد الشتايم التلقائي شغال (هيألف شتايم جديدة من الكلمات)")
